...
class ExistingVideoReachedWithContext(ExistingVideoReached):
    """--break-on-existing triggered"""

    def __init__(
        self,
        message="Encountered a video that is already in the archive, stopping due to --break-on-existing",
        extra_info=None,
    ):
    ...
    def __str__(self):
        return f"{self.message} | Additional Info: {self.extra_info}"
...
def update_url(url, *, query_update=None, **kwargs):
    ...
    if query_update:
        ...
        kwargs["query"] = urllib.parse.urlencode(
            {**urllib.parse.parse_qs(url.query), **query_update}, True
        )
...
def render_table(header_row, data, delim=False, extra_gap=0, hide_empty=False):
    ...
    def filter_using_list(row, filterArray):
        return [
            col
            for take, col in itertools.zip_longest(filterArray, row, fillvalue=True)
            if take
        ]
...
def scale_thumbnails_to_max_format_width(formats, thumbnails, url_width_re):
    ...
    * Update dimensions
    ...
    _keys = ("width", "height")
    max_dimensions = max(
        (tuple(format.get(k) or 0 for k in _keys) for format in formats), default=(0, 0)
    )
...
class FormatSorter:
    ...
    settings = {
        ...
        "res": {
            "type": "multiple",
            "field": ("height", "width"),
            "function": lambda it: (lambda l: min(l) if l else 0)(
                tuple(filter(None, it))
            ),
        },
        # Actual field names
        ...
        if key not in propObj:
            ...
            if key == "field":
                default = (
                    "preference"
                    if type == "extractor"
                    else (field,) if type in ("combined", "multiple") else field
                )
            elif key == "convert":
                default = (
                    "order"
        ...
        for item in sort_list:
            ...
            if match is None:
                raise ExtractorError(
                    'Invalid format sort string "%s" given by extractor' % item
                )
    ...
    def print_verbose_info(self, write_debug):
        ...
        if self._sort_extractor:
            write_debug(
                "Sort order given by extractor: %s" % ", ".join(self._sort_extractor)
            )
        write_debug(
            "Formats sorted by: %s"
            % ", ".join(
